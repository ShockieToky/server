 ...
 *   1 allié  → +8 % VIT
 *   2 alliés → +18 % VIT, +10 % DEF
 *   3 alliés → +30 % VIT, +20 % DEF, +15 % soin reçu
 */
class HeritageNomadePassive implements PassiveInterface
{
    ...
    public function apply(CombatContext $context): void
    {
        ...
        if ($count >= 3) {
            ...
            $context->activeEffects[]    = 'Héritage Nomade [3] : +30% VIT, +20% DEF, +15% soin';
        } elseif ($count >= 2) {
            $context->speedMultiplier   += 0.18;
            $context->defenseMultiplier += 0.10;
            $context->activeEffects[]    = 'Héritage Nomade [2] : +18% VIT, +10% DEF';
        } elseif ($count >= 1) {
